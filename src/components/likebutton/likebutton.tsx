...
import { likePostByID } from "@/utils/data";
...
export default function LikeButton({
  ...
  return (
    <div className={"flex gap-2 items-center"}>
      <button
        onClick={async () => {
          startTransition(() => setOptimisticLikes(1));
          await likePostByID(id);
        }}
      >
        <ClapIcon />
      </button>
      <p className={"pt-[2px]"}>{optimisticLikes}</p>
    </div>
  );
}

export function LikeButtonLink({ likes }: { likes: number }) {
  return (
    <div className={"flex gap-2 items-center"}>
